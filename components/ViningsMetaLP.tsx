...
// Recreation of start.colgrove.com/meta/ (Vinings Hair paid-social consult LP)
// on the EFFVIT fleet. Copy reproduced verbatim from source (Joe waived Meta
// ad-policy softening — Meta doesn't enforce it). The ONE carve-out kept is
// HIPAA: no browser Meta pixel on this health-intent form page (GTM is
// route-gated off /m/; attribution rides the GhlForm params + server-side CAPI).
...
export default function ViningsMetaLP({
  heroFormId,
  bottomFormId,
}: {
  heroFormId: string
  ...
  const beforeAfter = [
    { src: '/img/m/ba-2.webp', alt: 'Vinings hair restoration patient result' },
    { src: '/img/m/ba-3.webp', alt: 'Vinings hair restoration patient result' },
    { src: '/img/m/ba-1.webp', alt: 'Vinings hair restoration patient result' },
  ]
    ...
    <>
      ...
      <section ref={heroReveal.ref} className={heroReveal.className} style={{ background: '#fff', padding: '56px 24px 48px' }}>
        <div className="two-col" style={{ maxWidth: 1200, margin: '0 auto', alignItems: 'start', gap: 48 }}>
          <div>
            <span style={{ display: 'inline-block', background: '#f5efe0', color: NAVY, fontSize: 12, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', padding: '6px 14px', borderRadius: 40, marginBottom: 18 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 20, marginBottom: 28 }}>
              <a href="#hero-form" style={{ background: NAVY, color: '#fff', padding: '14px 26px', borderRadius: 40, fontSize: 13, fontWeight: 700, textDecoration: 'none', whiteSpace: 'nowrap' }}>
                Request Your Evaluation
              </a>
              <a href={PHONE_HAIR_HREF} style={{ display: 'flex', alignItems: 'center', gap: 8, color: HEADING, fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
          ...
          {/* Right: form card + patient-result gallery */}
          <div>
            <div id="hero-form" style={{ background: CARD_BG, border: `1px solid ${CARD_BORDER}`, borderRadius: 16, padding: '24px', marginBottom: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
              {beforeAfter.map(img => (
                <img key={img.src} src={img.src} alt={img.alt} style={{ width: '100%', borderRadius: 10, display: 'block' }} />
              ))}
            </div>
            <p style={{ textAlign: 'center', fontSize: 12, color: '#99a1af', margin: '8px 0 0' }}>Actual Patient Results &middot; Individual results vary</p>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div className="two-col" style={{ gap: 20, textAlign: 'left' }}>
            {[
              ...
              <div key={t} style={{ display: 'flex', gap: 14, background: CARD_BG, border: `1px solid ${CARD_BORDER}`, borderRadius: 12, padding: '20px 22px' }}>
                <Check />
                <div>
      ...
      {/* Permanent */}
      <section ref={permanentReveal.ref} className={permanentReveal.className} style={{ background: '#f1ede4', padding: '64px 24px', textAlign: 'center' }}>
        ...
        <div className="two-col" style={{ maxWidth: 1050, margin: '0 auto', gap: 48, alignItems: 'center' }}>
          <div>
            ...
            <p style={{ fontSize: 15, color: BODY, margin: '0 0 20px', lineHeight: 1.7 }}>
              Request your private evaluation with Vinings Surgery &amp; Hair Restoration Center&apos;s medical team.
              No obligation, no pressure &mdash; just a clear, honest picture of your options.
            </p>
